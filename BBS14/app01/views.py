...
def get_code(request):
    ...
    # img_obj = Image.new('RGB', (380, 35), get_random())
    # io_obj=BytesIO() # 生成一个内存管理器对象，也可以看成是文件句柄
    ...
    for i in range(5):
        ...
        code+=tmp
    print(code)
    # 随机验证码在登录的视图函数里面要用到，要比对，所以要找地方存起来并且其他视图函数也能拿到
...
def article_detail(request,username,article_id):
    '''
    应该需要校验username和article_id是否存在，但是这里先只考虑正确的情况
    默认不会瞎搞
    :param request:
    :param username:
    :param article_id:
...
def up_or_down(request):
    ...
    if request.is_ajax():
        ...
        if request.user.is_authenticated():
            ...
            # 2.判断当前文章是否是当前用户自己写的
            # 根据文章id查询文章对象，根据文章对象查作者，跟request.user比对
            ...
            if not article_obj.blog.userinfo == request.user:
                ...
                if not is_click:
                    ...
                    back_dic['code']=1001
                    back_dic['msg']='你已经点过了，不能再点了' # 这里可以做得更加的详细的，提示用户到底点了赞还是点了踩
            else:
                back_dic['code']=1002
...
def upload_image(request):
    # 用户写文章上传图片，也算静态资源，也应该放到media文件夹下
